// cotizaciones_semillas.js - Sistema de Cotizaciones Automatizado para Semillas
class SeedQuotationManager {
  constructor() {
    ...
    this.currentProductType = 'semillas'; // Fijo para semillas
    ...
    mainContent.innerHTML = `
      <div class="quote-controls">
        <div class="section-header">
          <div>
            <h2><i class="fas fa-file-invoice-dollar"></i> Sistema de Cotizaciones de Semillas</h2>
        ...
        <div class="single-page-quote">
          ...
          <div class="quote-form-panel">
            ...
            <div class="form-section">
              <h4><i class="fas fa-boxes"></i> Agregar Semillas</h4>
              
              <div class="search-container">
                <div class="search-bar">
                  <i class="fas fa-search"></i>
                  <input type="text" id="productSearch" placeholder="Buscar semillas por nombre...">
                  <button class="btn-icon" id="clearSearch">
                    <i class="fas fa-times"></i>
                  </button>
                </div>
                <div class="search-info">
                  <span id="availableProductsCount">Cargando semillas...</span>
                </div>
              </div>
              
              <div class="products-container" id="availableProductsContainer">
                <!-- Las semillas se cargarán aquí en tabla -->
              </div>
            </div>

            <div class="form-section">
              <h4><i class="fas fa-shopping-cart"></i> Semillas Seleccionadas</h4>
              <div class="selected-products-list" id="selectedProductsList">
                <div class="empty-state">
                  <i class="fas fa-cart-plus"></i>
                  <p>No hay semillas seleccionadas</p>
                  <small>Busca y selecciona semillas del inventario</small>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="preview-summary">
              <div class="summary-section">
                ...
                <div class="info-grid">
                  ...
                  <div class="info-item">
                    <span>Tipo:</span>
                    <strong id="summaryType">Semillas</strong>
                  </div>
                  <div class="info-item">
                    <span>Envío:</span>
              ...
              <div class="summary-section">
                <h4><i class="fas fa-list"></i> Semillas en Cotización</h4>
                <div class="products-summary" id="productsSummaryTable">
                  <p class="no-items">No hay semillas seleccionadas</p>
                </div>
              </div>
            ...
            <div class="modal-body">
              <div class="tax-discount-form">
                <div class="form-group">
                  ...
                  <small>Para semillas, generalmente 0%</small>
                </div>
      ...
      <style>
        /* (Todos los estilos CSS permanecen exactamente iguales) */
        .quote-controls {
  ...
  init() {
    console.log('🎯 Inicializando módulo de Cotizaciones de Semillas');
    window.seedQuotationManager = this;
    this.bindEvents();
  ...
  loadAvailableProducts() {
    ...
    if (products.length === 0) {
      container.innerHTML = `
        <div class="no-products">
          ...
          <small>Ve al módulo de semillas para agregar productos</small>
        </div>
      `;
      document.getElementById('availableProductsCount').textContent = '0 semillas disponibles';
      return;
    }

    // Filtrar solo semillas con stock
          ...
          <tbody>
            ${availableProducts.map(product => `
              <tr class="product-row" data-id="${product.id}">
                <td><strong>${product.commonName || product.nombre || 'Sin nombre'}</strong></td>
                <td><em>${product.scientificName || product.especie || ''}</em></td>
                <td>$${this.formatNumberWithCommas(product.precio || product.unitPrice || 0)}</td>
    ...
    document.getElementById('availableProductsCount').textContent =
      `${availableProducts.length} semillas disponibles`;
  ...
  addProductToQuote(productId) {
    ...
    if (existingItem) {
      ...
      existingItem.subtotal = (existingItem.unitPrice || existingItem.precio) * existingItem.quantity;
    } else {
      const newItem = {
        id: product.id,
        commonName: product.commonName || product.nombre || '',
        scientificName: product.scientificName || product.especie || '',
        classification: product.classification || '',
        availableMonths: product.availableMonths || '',
        seedsPerKilo: product.seedsPerKilo || 0,
        unitPrice: product.unitPrice || product.precio || 0,
        quantity: 1,
        subtotal: product.unitPrice || product.precio || 0,
  ...
  updateProductQuantity(productId, newQuantity) {
    ...
    if (item) {
      if (newQuantity >= 1) {
        ...
        item.subtotal = (item.unitPrice || item.precio) * item.quantity;
        this.updateSelectedProducts();
        this.updateSummary();
  ...
  updateSelectedProducts() {
    ...
    productCountBadge.textContent = `${this.quoteItems.length} ${this.quoteItems.length === 1 ? 'semilla' : 'semillas'}`;
  ...
  updateSummary() {
    ...
    document.getElementById('summaryType').textContent = 'Semillas';
    ...
    if (this.quoteItems.length === 0) {
      table.innerHTML = '<p class="no-items">No hay semillas seleccionadas</p>';
    ...
    table.innerHTML = this.quoteItems.map(item => `
      <div class="product-summary-item">
        <div>
          ...
          <small>${this.formatNumberWithCommas(item.quantity)} kg × $${this.formatNumberWithCommas((item.unitPrice || item.precio).toFixed(2))}</small>
        </div>
    ...
    this.defaultConditions.forEach((condition, index) => {
      ...
      conditionElement.innerHTML = `
        <div style="display: flex; gap: 10px; margin-bottom: 10px;">
          ...
          <button class="btn-icon-sm" onclick="window.seedQuotationManager.removeCondition(${index})" style="color: #f44336; background: #ffebee;">
            <i class="fas fa-trash"></i>
          </button>
        </div>
      `;
  ...
  addNewCondition() {
    ...
    conditionElement.innerHTML = `
      <div style="display: flex; gap: 10px; margin-bottom: 10px;">
        ...
        <button class="btn-icon-sm" onclick="window.seedQuotationManager.removeCondition(${newIndex})" style="color: #f44336; background: #ffebee;">
          <i class="fas fa-trash"></i>
        </button>
      </div>
    `;
  ...
  validateQuote() {
    ...
    if (this.quoteItems.length === 0) {
      window.app?.showNotification?.('Selecciona al menos una semilla', 'error');
      return false;
    }
  ...
  generateQuotation() {
    ...
    const quoteData = {
      ...
      items: this.quoteItems.map(item => ({
        ...item,
        subtotal: (item.unitPrice || item.precio) * item.quantity
      })),
...
<body>
  ...
  getPrintHTML(quoteData) {
    const itemsHTML = quoteData.items?.map((item, index) => `
      <tr>
        <td>${index + 1}</td>
        <td><strong>${item.commonName || 'Semilla'}</strong></td>
        ...
        <td>$${this.formatNumberWithCommas((item.unitPrice || item.precio || 0).toFixed(2))}</td>
    ...
    `).join('') || '<tr><td colspan="9" style="text-align: center; padding: 20px; color: #666;">No hay semillas en la cotización</td></tr>';
      ...
      <div class="print-container">
        ...
        <div class="print-quote-info">
          <div class="print-quote-header-row">
            <div class="print-quote-title-section">COTIZACIÓN DE SEMILLAS</div>
          </div>
...
window.SeedQuotationManager = SeedQuotationManager;
